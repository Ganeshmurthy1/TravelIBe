				...
				<div class="whiteBakground">
					<form class="form clearfix" name="hotelSummary">
						...
						<input type="hidden" name="ccy" value="{{basecurrencyname}}"
							...
							value="{{quoteuser.hotelquotationid}}" /> <input type="hidden"
							name="sky" value="{{searchKey}}" id="sky"> <input
							type="hidden" name="apiPro" value="{{apiProvider}}" id="apiPro">
						<input type="hidden" name="hCode" value="{{hotelCode}}" id="hCode">
						<input type="hidden" name="bCe" value="{{bookingCode}}" id="bCe">
						<input type="hidden" name="Units" value="{{noofrooms}}" id="Units">
						<s:if test="#session.isLabeling == true">
							...
							<s:else>
								<input type="hidden" name="ud" id="uid"
									value="<s:property value="%{#session.themeUserId}"/>">
								<input type="hidden" name="pmode" id="isagent" value="card">
								<input type="hidden" name="uname" id="uname"
									value="<s:property value="%{#session.themeUserName}"/>">
							</s:else>
 ...
 <s:if test="#session.isCardAcess == true">
	...
	<div id="radioDiv">
							 <span> <input type="radio" id="Cardoption"
										...
										</span> <label id="payoptiontext" class="cardText">
											(Card charges will be applicable)</label>
						</div>
	 </s:if>
 </s:if>
  <s:if test="#session.isLabeling != true"> --%>
						...
						<div class="col-sm-12 clearfix">
							<div class="form-group clearfix">
								...
								<s:else>
									<s:if
										test="#session['agent'] != null &&#session.isCorporate != true">
										<div class="input-group butonRight">
