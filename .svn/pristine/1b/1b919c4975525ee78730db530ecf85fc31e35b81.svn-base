...
app.controller('busSummaryCtrl', function($scope,$location,busServices,$http,$modal,$log,$window,transporter,localStorageService) {
	$scope.init = function(){
		...
		$scope.blockingData = {};
		$scope.rmData = localStorageService.get('rmblockData');
	...
	$scope.isCor=angular.element('#isCorp').val();
	...
	$scope.bookSummary = function(){
		...
		//PassengerDetails End
			...
			//Cancellation Policy
			 ...
			 if($scope.isCor=='true' || $scope.isCor== true){
					
				 $scope.callRmDetailApi($scope.rmData);	
				} 
	 ...
	 $scope.callRmDetailApi = function(rmData){
		 
		  busServices.busRmbook(rmData).then(function(response){
