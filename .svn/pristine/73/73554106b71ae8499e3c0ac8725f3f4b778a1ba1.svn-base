...
<!-- START: FOOTER -->
<s:if test="#session.isFooterEnabled == true">
	<section id="footer">
		<footer>
				...
				<div class="container clear-padding">
					...
					<div class="col-md-4 col-sm-6 links">
						...
						<ul>
							...
							<s:if test="#session.isB2E == true">
								<s:if test="#session['agent'] == null">
									<li id="login"><a data-toggle="modal"
										data-target="#welcomeagent" ng-click="corporatelogin()"
										class="b2bHAnd"> Corporate Login</a> <!-- data-toggle="modal" data-target="#welcomeagent" -->
										<div class="featherlight light" style="display: none;">
					...
					<s:if test="#session.isContactUsEnabled == true">
						<div class="col-md-4 col-sm-6 contact-box">
							<h4>Contact Us</h4>
							<p class="clearfix">
								<i class="tayyarah-home4"></i> <span> # 19 "The
									Oyster",2nd floor, Nandi durga road, Jaymahal extension,
									Bengaluru-560046</span>
							</p>
							<p class="clearfix">
					...
					<div class="col-md-12 text-center we-accept">
						...
						<ul class="policy-links">
							<s:if test="#session.isPrivacyPolicyEnabled == true">
								<li><a href="#/Privacypolicy">Privacy policy</a></li>
							</s:if>
							<s:if test="#session.isTermsEnabled == true">
								<li><a href="#/TermsandConditions">Terms and Conditions</a></li>
							</s:if>
							<li><a href="#/Cancellation">Cancellation / Refund
									policy</a></li>
				...
				<div id="formwrapper">
					<div id="form-login">
						...
						<form class="form-horizontal"
							data-ng-submit="loginWhiteLabelClick()">
...
<script>
	// Fixed header
	//-----------------------------------------------
	headerTopHeight = $(".header-top").outerHeight(), headerHeight = $(
					...
					function() {
						if (($(".header.fixed:not(.fixed-before)").length > 0)
							...
							if (($(this).scrollTop() > headerTopHeight
								...
								$(".header.fixed:not(.fixed-before)").addClass(
										'animated object-visible fadeInDown');

							} else {
								$("body").removeClass("fixed-header-on");

								$(".header.fixed:not(.fixed-before)")
										.removeClass(
												'animated object-visible fadeInDown fixheader');
							}
	...
	$(window).scroll(
			function() {
				if (($(".header.fixed.fixed-before").length > 0)
						&& !($(".transparent-header .slideshow").length > 0)) {
					if (($(this).scrollTop() > headerTopHeight)
							&& ($(window).width() > 767)) {
						$("body").addClass("fixed-header-on");
						$(".header.fixed.fixed-before").addClass(
								'object-visible');

					} else {
						$("body").removeClass("fixed-header-on");

						$(".header.fixed.fixed-before").removeClass(
								'object-visible');
					}
...
<div class="livechat" style="position: fixed; bottom: 0px; right: 0px;">
	...
	<script type="text/javascript">
		...
		script.async = true;
		script.type = "text/javascript";
		var src = "https://tayyarah.com/livezilla/server.php?a=7353b&rqst=track&output=jcrpt&nse="
				+ Math.random();
		setTimeout(
